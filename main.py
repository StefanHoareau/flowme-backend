...
async def get_interface():
    ...
    return HTMLResponse(get_enhanced_interface_html())

def get_enhanced_interface_html():
...
async def get_transition_suggestions():
    """Suggestions de transition entre états"""
    try:
        return {
            "status": "success",
            "message": "Endpoint de transition disponible",
            "example": "POST /transition avec current_state et desired_outcome"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def suggest_state_transition(request: TransitionRequest):
    """Suggère des transitions d'états"""
    try:
        transition = suggest_transition(
