...
async def get_interface():
    ...
    # Si fichier custom existe, l'utiliser
    ...
    # Interface par défaut intégrée et améliorée
    return HTMLResponse(get_enhanced_default_interface())

def get_enhanced_default_interface():
    """Interface FlowMe par défaut avec toutes les fonctionnalités"""
    return f"""
    <!DOCTYPE html>
    <html lang="fr">
    <head>
    ...
    <body>
        ...
        <script>
            ...
            async function sendMessage() {{
                ...
                try {{
                    // Analyser le message avec FlowMe
                    const response = await fetch(`${{API_BASE}}/analyze/enhanced`, {{
                        method: 'POST',
                        headers: {{ 'Content-Type': 'application/json' }},
                        body: JSON.stringify({{ 
                            message: message,
                            session_history: stateHistory,
                            deep_analysis: true
                        }})
                    }});
            ...
            function addAIMessage(data) {{
                ...
                message.innerHTML = `
                    ...
                    <div class="message-meta">
                        État: ${{data.detected_state}} - ${{data.state_info.name}} 
                        (${{data.state_info.famille_symbolique}})
                    </div>
                    <div class="state-info">
                        <strong>Posture:</strong> ${{data.state_info.posture_adaptative}}
                    </div>
                `;
            ...
            function updateCurrentState(stateId, stateInfo) {{
                ...
                document.getElementById('current-state-name').textContent = stateInfo.name;
                document.getElementById('current-state-family').textContent = stateInfo.famille_symbolique;
            }}
    ...
    try:
        ...
        return {
            ...
            "state_info": {
                ...
                "famille_symbolique": state_info["famille_symbolique"],
                "mot_cle": state_info["mot_cle"],
                "tension_dominante": state_info["tension_dominante"],
                "posture_adaptative": state_info["posture_adaptative"],
                "etats_compatibles": state_info["etats_compatibles"]
            },
            "advice": advice,
...
async def analyze_message_enhanced(request: FlowAnalysisRequest):
    ...
    try:
        ...
        return {
            ...
            "flow_tendency": analysis["flow_tendency"],
...
async def get_all_states():
    ...
    try:
        ...
        for state_id in range(1, 65):
            ...
            famille = state_info["famille_symbolique"]
    ...
    try:
        ...
        for comp_id in compatible_states:
            ...
            compatible_details.append({
                "id": comp_id,
                "name": comp_info["name"],
                "famille_symbolique": comp_info["famille_symbolique"]
            })
        ...
        return {
            ...
            "famille_info": FAMILLE_SYMBOLIQUE.get(state_info["famille_symbolique"], {}),
            "timestamp": datetime.now().isoformat()
        }
    ...
    try:
        ...
        health_data = {
            ...
            "test_detection": {
                ...
                "state_name": FLOWME_STATES[test_state]["name"]
            },
...
# Test au démarrage
try:
    from flowme_states_detection import FLOWME_STATES, FAMILLE_SYMBOLIQUE
...
# Ajoutez ces corrections à votre main.py existant

# Modèle pour l'interface existante
class LegacyAnalyzeRequest(BaseModel):
    message: str
    user_id: Optional[str] = "anonymous"

# Endpoint pour compatibilité avec l'interface existante
@app.post("/analyze")
async def analyze_message_legacy(request: LegacyAnalyzeRequest):
    """Endpoint compatible avec l'interface FlowmeLocal v3 existante"""
    try:
        # Détecter l'état avec le message simple
        ...
        # Format de réponse compatible avec l'interface
        return {
            "status": "success",
            ...
            "mot_cle": state_info["mot_cle"],
            "advice": advice,
            "conseil": state_info["conseil_flowme"],
            "posture": state_info["posture_adaptative"],
            "compatible_states": state_info["etats_compatibles"][:4],  # Limiter à 4
            "timestamp": datetime.now().isoformat(),
            "confidence": 0.85
        }
        
    except Exception as e:
        # Retourner une réponse d'erreur compatible
        return {
            "status": "error",
            "state": 1,
            "state_name": "Présence",
            "famille": "Écoute subtile",
            "advice": f"Je rencontre une difficulté technique : {str(e)}. Pouvez-vous reformuler votre message ?",
            "error": str(e)
        }
...
async def log_requests(request, call_next):
    """Logger les requêtes pour débugger"""
    ...
    try:
        ...
        print(f"✅ Réponse: {response.status_code} en {duration:.2f}s")
        return response
    except Exception as e:
        duration = (datetime.now() - start_time).total_seconds()
        print(f"❌ Erreur: {str(e)} après {duration:.2f}s")
        raise
